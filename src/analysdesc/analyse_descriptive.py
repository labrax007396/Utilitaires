...
def MissingValues(x: pd.DataFrame, dico_model:dict):
    import missingno as msno

    dico_map = {df['nom']:df['description'] for df in dico_model['facteurs'].values()}
    dico_map[dico_model['tag_name']] = dico_model['description']

    x_cpy = x.copy(deep=True)
    x_cpy.rename(columns=dico_map, inplace=True)

    df_missing = x_cpy.apply(num_missing, axis=0)
        
    print("% de valeurs manquantes par colonne:")
    print(df_missing)
    fig = msno.matrix(x_cpy,figsize=(10,5), fontsize=12)
...
def plot_series(x,**kwargs):
    ...
    fig = go.Figure()
    for c in x.columns:
        fig.add_trace(go.Scatter(x=x.index, y=x[c],
                            mode='lines',
                            name=c))
    fig.show()
...
def correlation(data: pd.DataFrame, dico_model:dict):

    from plotly.subplots import make_subplots
    import plotly.graph_objects as go


    dico_map = {df['nom']:df['description'] for df in dico_model['facteurs'].values() if df['type']=='num'}
    dico_map[dico_model['tag_name']] = dico_model['description']

    var_numerique = [v for v in data.columns if data[v].dtypes != 'object']
    data_num = data[var_numerique]

    data_num.rename(columns=dico_map, inplace=True)
    var_numerique = [v for v in data_num.columns if data_num[v].dtypes != 'object']
...
def correlogramme(data: pd.DataFrame,dico_model:dict):
